//LIO.space - Starship Sript [Version 2.0.1]
...
function init {
    ...
    set landingZone to latlng((-0.117917521127475), -74.4865094025858).  //desired landing zone, latitude on the left and longitude on the right
    set ap to 20000.
...
function windowcarefull {
    ...
    LOCAL titlegui IS gui:ADDLABEL("CAREFULL!").
    set titlegui:STYLE:TEXTCOLOR to RED.
    set titlegui:style:align to "CENTER".
    SET titlegui:style:fontsize to 20.
    set titlegui:style:hstretch to true.
    LOCAL text1gui IS gui:ADDLABEL("The kOS code is still under development, if you continue, you alone are responsible for the loss of your ship. Please tell me on my discord if you have any problem :").
    set text1gui:style:align to "CENTER".
    SET text1gui:style:fontsize to 15.
    set text1gui:style:textcolor to white.
    set text1gui:style:hstretch to true.
    LOCAL text2gui IS gui:ADDLABEL("https://discord.gg/FQpaTxReDn").
    set text2gui:style:align to "CENTER".
    SET text2gui:style:fontsize to 15.
    set text2gui:style:textcolor to white.
    set text2gui:style:hstretch to true.
    local okgui to gui:ADDBUTTON("OK").
    gui:SHOW().

    set donegui to false.
    when (okgui:TAKEPRESS) then {
        gui:HIDE().
...
function gravityturn {
    ...
    until apoapsis > ap - verticalSpeed * 10 {
        ...
        if trueRadar > ap/2 and availabletwr > 1 and eng3twr > 1 {
            if ag2 = false {
                eng2:shutdown.
                set LASTEVENT to "ENG2 SHUTDOWN".
                ag3 off. //This is an indicator that will be used later if the engines 2 are off or not.
    ...
    until trueRadar > ap {
        ...
        if ship:verticalSpeed < 60 {
            set targettwr to MIN(1, targettwr + 0.1).
        } 

        if ship:verticalSpeed > 50 {
            set targettwr to MAX(0, targettwr - 0.1).
        } 

        if targettwr < 0.3 {
            set targettwr to 0.3.
        }
